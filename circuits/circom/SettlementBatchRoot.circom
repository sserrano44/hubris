...
template Poseidon2HashPair() {
    ...
    // Initial state and initial external matrix multiplication.
    signal extState[5][3];
    extState[0][0] <== left + left + right;
    extState[0][1] <== left + right + right;
    extState[0][2] <== left + right;

    component extPow[4][3];
    for (var round = 0; round < 4; round++) {
        for (var i = 0; i < 3; i++) {
            extPow[round][i] = Pow5();
            extPow[round][i].in <== extState[round][i] + RC_EXTERNAL[round][i];
        }

        extState[round + 1][0] <== extPow[round][0].out + extPow[round][0].out + extPow[round][1].out + extPow[round][2].out;
        extState[round + 1][1] <== extPow[round][0].out + extPow[round][1].out + extPow[round][1].out + extPow[round][2].out;
        extState[round + 1][2] <== extPow[round][0].out + extPow[round][1].out + extPow[round][2].out + extPow[round][2].out;
    }

    signal intState[57][3];
    intState[0][0] <== extState[4][0];
    intState[0][1] <== extState[4][1];
    intState[0][2] <== extState[4][2];

    component intPow[56];
    for (var round = 0; round < 56; round++) {
        intPow[round] = Pow5();
        intPow[round].in <== intState[round][0] + RC_INTERNAL[round];

        intState[round + 1][0] <== intPow[round].out + intPow[round].out + intPow[round].out + intState[round][1] + intState[round][2];
        intState[round + 1][1] <== intPow[round].out + intState[round][1] + intState[round][1] + intState[round][1] + intState[round][2];
        intState[round + 1][2] <== intPow[round].out + intState[round][1] + intState[round][2] + intState[round][2] + intState[round][2] + intState[round][2];
    }

    signal tailState[5][3];
    tailState[0][0] <== intState[56][0];
    tailState[0][1] <== intState[56][1];
    tailState[0][2] <== intState[56][2];

    component tailPow[4][3];
    for (var round = 0; round < 4; round++) {
        var rcIndex = round + 4;
        for (var i = 0; i < 3; i++) {
            tailPow[round][i] = Pow5();
            tailPow[round][i].in <== tailState[round][i] + RC_EXTERNAL[rcIndex][i];
        }

        tailState[round + 1][0] <== tailPow[round][0].out + tailPow[round][0].out + tailPow[round][1].out + tailPow[round][2].out;
        tailState[round + 1][1] <== tailPow[round][0].out + tailPow[round][1].out + tailPow[round][1].out + tailPow[round][2].out;
        tailState[round + 1][2] <== tailPow[round][0].out + tailPow[round][1].out + tailPow[round][2].out + tailPow[round][2].out;
    }
...
template SettlementBatchRoot(maxActions) {
    ...
    component start0 = Poseidon2HashPair();
    start0.left <== batchId;
    start0.right <== hubChainId;
    acc[0] <== start0.out;

    component start1 = Poseidon2HashPair();
    start1.left <== acc[0];
    start1.right <== spokeChainId;
    acc[1] <== start1.out;

    component start2 = Poseidon2HashPair();
    start2.left <== acc[1];
    start2.right <== actionCount;
    acc[2] <== start2.out;

    for (var i = 0; i < maxActions; i++) {
        steps[i] = Poseidon2HashPair();
