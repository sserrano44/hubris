...
const INTERNAL_ROUNDS = 56;
const EXTERNAL_HALF_ROUNDS = EXTERNAL_ROUNDS / 2;

const RC_EXTERNAL: readonly (readonly [bigint, bigint, bigint])[] = [
  [
...
const RC_INTERNAL: readonly bigint[] = [
  ...
  0x198d425a45b78e85c053659ab4347f5d65b1b8e9c6108dbe00e0e945dbc5ff15n,
...
export function hashPair(left: bigint, right: bigint): bigint {
  const [s0] = poseidon2Permute3([toField(left), toField(right), 0n]);
  return s0;
}

export function poseidon2Permute3(state: readonly [bigint, bigint, bigint]): [bigint, bigint, bigint] {
  let current: [bigint, bigint, bigint] = [toField(state[0]), toField(state[1]), toField(state[2])];

  current = matmulExternal(current);

  for (let round = 0; round < EXTERNAL_HALF_ROUNDS; round++) {
    const rc = RC_EXTERNAL[round]!;
    current = [
      pow5(mod(current[0] + rc[0])),
      pow5(mod(current[1] + rc[1])),
      pow5(mod(current[2] + rc[2]))
    ];
    current = matmulExternal(current);
  }

  for (let round = 0; round < INTERNAL_ROUNDS; round++) {
    current = [pow5(mod(current[0] + RC_INTERNAL[round]!)), current[1], current[2]];
    current = matmulInternal(current);
  }

  for (let round = EXTERNAL_HALF_ROUNDS; round < EXTERNAL_ROUNDS; round++) {
    const rc = RC_EXTERNAL[round]!;
    current = [
      pow5(mod(current[0] + rc[0])),
      pow5(mod(current[1] + rc[1])),
...
function pow5(value: bigint): bigint {
  ...
  const fourth = mod(squared * squared);
  return mod(fourth * value);
}

function matmulExternal(state: readonly [bigint, bigint, bigint]): [bigint, bigint, bigint] {
...
function matmulInternal(state: readonly [bigint, bigint, bigint]): [bigint, bigint, bigint] {
  const [s0, s1, s2] = state;
  const sum = mod(s0 + s1 + s2);
  return [
    mod(s0 + s0 + s0 + s1 + s2),
    mod(s0 + s1 + s1 + s1 + s2),
    mod(sum + s2 + s2 + s2)
