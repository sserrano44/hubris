...
export function poseidon2Permute3(state) {
    let current = [toField(state[0]), toField(state[1]), toField(state[2])];
...
export function deriveActionIds(batch) {
    const ids = [];
...
export function computeActionsRoot(batch) {
